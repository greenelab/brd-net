...
def rebalance(plier_df, classifier_df, fraction=.2):
    ...
    Arguments
    ---------
    plier_df: pandas.DataFrame
        The dataframe to train PLIER
    classifier_df: pandas.DataFrame
        The dataframe used in training the classifier
    fraction: float
    ...
    -------
    plier_df: pandas.DataFrame
        The newly rebalanced version of the plier_df dataframe passed
    classifier_df: pandas.DataFrame
        The newly rebalanced version of the classifier_df dataframe passed
    '''
    # Pass studies back and forth if the change will even out the fraction more

    plier_samples = plier_df.columns
    classifier_samples = classifier_df.columns

    plier_study_counter = get_study_counts(plier_samples)
    classifier_study_counter = get_study_counts(classifier_samples)
    ...
    all_study_df = plier_df.join(classifier_df)
    new_plier_df = get_df_subset_by_study(all_study_df, plier_study_list)
    new_classifier_df = get_df_subset_by_study(all_study_df, classifier_study_list)

    return new_plier_df, new_classifier_df
...
if __name__ == '__main__':
    ...
    subset_plier_healthy_df, subset_classifier_healthy_df = rebalance(subset_plier_healthy_df,
                                                                      subset_classifier_healthy_df)
    subset_plier_disease_df, subset_classifier_disease_df = rebalance(subset_plier_disease_df,
